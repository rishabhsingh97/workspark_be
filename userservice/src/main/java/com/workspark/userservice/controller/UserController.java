...
public class TenantUserControllerImpl implements TenantUserController {
     ...
     */
    @Override
    public ResponseEntity<BaseRes<BasePageRes<SignInResponse>>> getAllTenantUsers(int page, int size) {
        log.info("Fetching all tenant users with pagination: page={}, size={}", page, size);

        BasePageRes<SignInResponse> response = tenantUserService.getAllUsers(page, size);
